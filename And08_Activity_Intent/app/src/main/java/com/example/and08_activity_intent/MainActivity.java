...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    //onStart에서 find등의 작업을 수행 예정. (setContentView라는 메소드가 실행 이후 ->onStart())
    @Override
    protected void onStart() {
        ...
        //암시적 인텐트 : activity => action(내가 없는 기능)으로 사욯함.
        edt1.setOnClickListener(v -> {
            Intent intent = new Intent(Intent.ACTION_DIAL, Uri.parse( "tel:/" + edt1.getText().toString() ));
            startActivity(intent);
        });

        btn2.setOnClickListener(this);
        btn3.setOnClickListener(this); // implements View.OnClickListener
    ...
    public void onClick(View v) {
        if(v.getId() == R.id.btn2) {
            ...
            intent.putExtra(SearchManager.QUERY, edt2.getText().toString()); //putExtra("query", "검색어");
            startActivity(intent);
